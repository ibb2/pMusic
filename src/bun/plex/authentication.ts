...
import type { ServerChangeResult } from "../../shared/types";
import type {
  Connection,
  PlexConnectionMode,
  PlexLibrary,
...
class Authentication {
  ...
  /**
   * Validates a destination without disturbing the active server, then commits
   * the server and library selection together. The callback is deliberately
   * run after validation but before persistence so playback can be stopped at
  ...
  async changeServer(
    ...
    const previousServer =
      this.selectedServer ||
      this.store.get<PlexServer>("selectedServer") ||
      null;
    const previousServerId = previousServer?.clientIdentifier || null;
    const previousLibraries = this.selectedLibraries;
    const previousConnection = this.getLastKnownGoodConnection();
    let commitStarted = false;

    if (previousServerId === server.clientIdentifier) {
      return {
        changed: false,
        previousServerId,
        server: previousServer,
        error: "This Plex server is already selected",
      };
    }

    try {
      const candidates = this.getConnectionCandidates(mode, server, null, []);
      const reachable = await findReachablePlexConnection(
        candidates,
        (connection) => this.canReachConnection(connection.uri, server),
      );
      if (!reachable)
      ...
      if (commitStarted) {
        this.selectedServer = previousServer;
        this.selectedLibraries = previousLibraries;
        if (previousServer) this.store.set("selectedServer", previousServer);
        else this.store.delete("selectedServer");
        if (previousLibraries)
          this.store.set("selectedLibraries", previousLibraries);
        else this.store.delete("selectedLibraries");
        this.clearLastKnownGoodConnection();
        if (previousConnection)
          this.setLastKnownGoodConnection(previousConnection);
      }
      return {
        changed: false,
        previousServerId,
  ...
  private async refreshSelectedServer(): Promise<PlexServer | null> {
    ...
    try {
      ...
      const refreshedServer = servers.find(
        (server) => server.clientIdentifier === selectedServer.clientIdentifier,
      );
