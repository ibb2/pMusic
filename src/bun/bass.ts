...
type BassLibrary = {
  symbols: {
    ...
    BASS_ChannelSetAttribute: (handle: number, attribute: number, value: number) => boolean
    BASS_ChannelGetLength: (handle: number, mode: number) => bigint
    BASS_ChannelGetPosition: (handle: number, mode: number) => bigint
    BASS_ChannelSetPosition: (handle: number, position: bigint, mode: number) => boolean
...
export class BassManager {
  ...
  private pluginStatuses: Array<{ name: string; path: string; loaded: boolean; error: string | null }> =
    []
  ...
  getStatus(): BassStatus {
    return {
      ...
      version: this.library ? this.formatVersion(this.library.symbols.BASS_GetVersion()) : null,
  ...
  setVolume(volume: number): void {
    ...
    if (!this.library || !this.streamHandle) return
    this.library.symbols.BASS_ChannelSetAttribute(this.streamHandle, BASS_ATTRIB_VOL, this.volume)
  }
  ...
  private load(): void {
    if (!this.libraryPath) {
      this.loadError = 'No BASS library is available for this platform or architecture.'
      return
    }
    ...
    try {
      this.library = dlopen(this.libraryPath, {
        ...
        BASS_StreamCreateURL: {
          args: [FFIType.cstring, FFIType.u32, FFIType.u32, FFIType.ptr, FFIType.ptr],
          returns: FFIType.u32
        },
        BASS_StreamFree: {
          args: [FFIType.u32],
      ...
      this.initialized = this.library.symbols.BASS_Init(-1, 44100, 0, null, null)
      if (!this.initialized) {
  ...
  private loadPlugins(): void {
    ...
    this.pluginStatuses = this.pluginCandidates().map((plugin) => {
      ...
      const handle = this.library!.symbols.BASS_PluginLoad(this.toCString(plugin.path), 0)
      if (!handle) {
        return {
          ...plugin,
          loaded: false,
  ...
  private getPosition(): number {
    ...
    const position = this.library.symbols.BASS_ChannelGetPosition(this.streamHandle, BASS_POS_BYTE)
  ...
  private libraryCandidates(): string[] {
    ...
    return roots.map((root) => {
      ...
      if (process.platform === 'win32') {
        return join(root, 'win32', process.arch === 'ia32' ? 'ia32' : 'x64', 'bass.dll')
      }
  ...
  private pluginCandidates(): Array<{ name: string; path: string }> {
    ...
    return pluginNames.map((name) => {
      const candidates = roots.map((root) => {
        if (process.platform === 'darwin') return join(root, 'macos', `libbass${name}.dylib`)

        if (process.platform === 'win32') {
          return join(root, 'win32', process.arch === 'ia32' ? 'ia32' : 'x64', `bass${name}.dll`)
        }
