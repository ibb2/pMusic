...
class Authentication {
  ...
  public generateClientIdentifier(): string {
    ...
    let clientIdentifier = this.store.get("clientIdentifier") as
      | string
      | undefined;
    if (clientIdentifier) {
  ...
  public async checkPinStatus(id: string) {
    ...
    if (data["authToken"]) {
      ...
      this.store.set(
        "plexUserAccessToken",
        this.encrypt(this.plexUserAccessToken),
      );
  ...
  public isUserSignedIn(): boolean {
    ...
    const authenticationToken = this.store.get("plexUserAccessToken") as
      | string
      | undefined;
  ...
  public async getServers(): Promise<PlexServer[]> {
    ...
    const servers: PlexServer[] = data.filter(
      (s) => s.product == "Plex Media Server",
    );
    return servers;
  }
  ...
  public isServerSelected(): boolean {
    const selectedServer = this.store.get("selectedServer") as
      | string
      | undefined;
  ...
  private setUserInformation() {
    this.plexClientId = this.generateClientIdentifier();
    try {
      this.plexUserAccessToken = this.decrypt(
        this.store.get("plexUserAccessToken") as string,
      );
      ...
      this.selectedServer =
        selectedServer != undefined
          ? (JSON.parse(selectedServer) as PlexServer)
          : null;
      const selectedLibraries = this.store.get("selectedLibraries") as
        | string
        | undefined;
      this.selectedLibraries =
        selectedLibraries != undefined
          ? (JSON.parse(selectedLibraries) as any[])
          : null;
    } catch (e) {
